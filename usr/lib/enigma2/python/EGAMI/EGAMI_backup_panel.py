...
HaveGZkernel = True
if getMachineBuild() in ('et1x000', 'vuuno4k', 'vuultimo4k', 'vusolo4k', 'spark', 'spark7162',
                         'hd51', 'hd52', 'sf4008', 'dags7252', 'gb7252', 'vs1500',
                         'h7', 'xc7439', '8100s'):
    HaveGZkernel = False
...
#        m = eEGAMI.getInstance().checkkernel()
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions'], {'red': self.closeMenu,'cancel': self.closeMenu,'yellow': self.backuP,'green': self.restorE})
...
class EGAMIRestorePanel_Step1(Screen, ConfigListScreen):
    ...
    def updateList(self):
        blist = [
         'Password', 'Devices', 'Network', 'Cron', 'Password', 'Keymaps', 'Nfs', 'Openvpn', 'Inadyn', 'Httpd', 'Uninstall files', 'Settings Channels Bouquets', 'Satellites Terrestrial', 'SoftCams', 'Scripts', 'Bootlogo', 'Plugins Extensions', 'System Plugins']
        for x in blist:
    ...
    def Continue(self):
        mylist = [
         'start', 'extract', 'lib', 'lib/firmware', 'usr/lib', 'usr/bin', 'etc']
...
#        m = eEGAMI.getInstance().checkkernel()
        ...
        self['label2'] = Label(_('Here is the list of mounted devices in Your STB\nPlease choose a device where You would like to keep Your backup:'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'green': self.saveMysets,'red': self.close,'yellow': self.yellow,'back': self.close})
        self.deviceok = True
        self.updateList()
    ...
    def doFullBackup(self, DIRECTORY):
        ...
        if 'ubi' in self.ROOTFSTYPE.split():
            ...
            if 'tar.bz2' in self.ROOTFSTYPE.split() or SystemInfo['HaveMultiBoot']:
                self.MKFS = '/bin/tar'
        ...
        self.EXTRA = '%s/EGAMI_fullbackup_%s/%s/%s' % (self.DIRECTORY, self.MODEL, self.DATE, self.IMAGEFOLDER)
        self.EXTRAOLD = '%s/EGAMI_fullbackup_%s/%s/%s' % (self.DIRECTORY, self.MODEL, self.DATE, self.MODEL)
        self.message = "echo -e '\n"
    ...
    def doFullBackupCB(self):
        ...
        if SystemInfo['HaveMultiBoot'] and self.list[self.selection] == 'Recovery':
            ...
            if self.MODEL in ('vusolo4k', 'vuultimo4k', 'vuduo2', 'vusolo2', 'vusolo',
                              'vuduo', 'vuultimo', 'vuuno'):
                cmdlist.append('echo "This file forces a reboot after the update." > %s/reboot.update' % self.MAINDEST)
            else:
                if self.MODEL in ('vuzero', 'vusolose', 'vuuno4k', 'xpeedlxpro', 'sf98'):
        ...
        if self.MODEL in ('gbquad', 'gbquadplus', 'gb800ue', 'gb800ueplus', 'gbultraue',
                          'twinboxlcd', 'twinboxlcdci', 'singleboxlcd', 'sf208',
                          'sf228'):
