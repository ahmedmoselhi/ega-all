...
class EGDecodingSetup(ConfigListScreen, Screen):
    ...
    def saveAndExit(self):
        if config.usage.dsemudmessages.value is not False:
            ...
            if config.usage.dsemudmessages.value is not True:
        ...
        if config.usage.messageYesPmt.value is not False:
            ...
            if config.usage.messageYesPmt.value is not True:
    ...
    def updateList(self):
        ...
        for plugin in self.pluginlist:
            if plugin.icon is None:
                ...
                png = plugin.icon
            res = (
             plugin.name, plugin.description, png, plugin)
...
class EGKernelModules(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'red': self.close,'green': self.saveMyconf,
           'yellow': self.showMod,
           'back': self.close
           })
        self.updateList()
    ...
    def updateList(self):
        ...
        self.pcscd = NoSave(ConfigYesNo(default=False))
        if fileExists('/usr/bin/egami_modules.sh'):
            ...
            for line in f.readlines():
                ...
                elif line.find('pcscd') != -1:
                    self.pcscd.value = True
        ...
        self.list.append(res)
        res = getConfigListEntry(_('PCSC Smart CardReader:'), self.pcscd)
        self.list.append(res)
        self['config'].list = self.list
        self['config'].l.setList(self.list)
...
class EGAMISpeedUpWizard(Screen, ConfigListScreen):
    ...
    def __init__(self, session, firstrun=False):
        ...
        self['key_green'] = Label(_('Save'))
        self['key_red'] = Label(_('Exit'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'green': self.saveMypoints,'red': self.close,
           'back': self.close
           })
        self.pluglist = []
        ...
        for plug in self.pluglist:
            if plug[0] == name:
                ...
                if what == 'Enabled' and installed == False:
                    ...
                    if what == 'Disabled' and installed == True:
...
class EGAMISkinWizard(Screen, ConfigListScreen):
    ...
    def __init__(self, session, firstrun=False):
        ...
        self.pluglist = [
         [
          'PinguTM Full HD', 'http://enigma-spark.com/egami/skins/PFHD.tar.gz'],
         [
    ...
    def checkInst(self, name):
        ...
        if name in 'http://enigma-spark.com/egami/skins/PFHD.tar.gz':
            ...
            if name in 'http://enigma-spark.com/egami/skins/NGFHD.tar.gz':
