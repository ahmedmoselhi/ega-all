...
from Components.Console import Console as CompnentConsole
...
class EGPcscdConfig(ConfigListScreen, Screen):
    ...
    def __init__(self, session):
        Screen.__init__(self, session)
        Screen.setTitle(self, _('Pcscd USB cardreader setup'))
        ...
        self.Console = CompnentConsole()
        self.my_vpn_active = False
        self.my_pcscd_run = False
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'ok': self.close,'back': self.close,'red': self.UninstallCheck,'green': self.PcscdStartStop,'yellow': self.activatePcscd})
        self.service_name = 'pcsc-lite'
    ...
    def checkNetworkState(self, str, retval, extra_args):
        if 'Collected errors' in str:
            ...
            if not str:
                ...
                cmd1 = 'opkg update'
                self.CheckConsole = CompnentConsole()
    ...
    def doInstall(self, callback, pkgname):
        ...
        self.Console.ePopen('/usr/bin/opkg install ' + pkgname + ' pcsc-lite-lib --force-reinstall', callback)
    ...
    def doRemove(self, callback, pkgname):
        ...
        self.Console.ePopen('/usr/bin/opkg remove ccid pcsc-lite-lib ' + pkgname + ' --force-depends --force-remove --autoremove', callback)
    ...
    def PcscdStartStop(self):
        if not self.my_pcscd_run:
            self.Console.ePopen('/etc/init.d/pcscd start', self.StartStopCallback)
        else:
            if self.my_pcscd_run:
                self.Console.ePopen('/etc/init.d/pcscd stop', self.StartStopCallback)

    def StartStopCallback(self, result=None, retval=None, extra_args=None):
        time.sleep(3)
        self.updateService()

    def activatePcscd(self):
        if fileExists('/etc/rc2.d/S20pcscd'):
            self.Console.ePopen('update-rc.d -f pcscd remove', self.StartStopCallback)
        else:
            self.Console.ePopen('update-rc.d -f pcscd defaults', self.StartStopCallback)
