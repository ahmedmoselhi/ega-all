...
def menu(menuid, **kwargs):
    if menuid == 'mainmenu':
        return [(_('EGAMI News'), main, 'EGAMINews', 1)]
    return []
...
def Plugins(**kwargs):
    return [
     PluginDescriptor(name='EGAMINews', where=PluginDescriptor.WHERE_MENU, description=_('Latest news about EGAMI Images'), fnc=menu),
...
class EGAMIMainNews(Screen):
    ...
    def __init__(self, session, args=0):
        ...
        self.list = [
         (
          'loading', _('Please wait ...'), self.png_info)]
...
class UpdateEGAMI(Screen):
    ...
    def __init__(self, session, args=None):
        Screen.__init__(self, session)
        self.sliderPackages = {'ceryon-dvb-modules': 1,'broadmedia-dvb-modules': 2,'ini-dvb-modules': 3,'enigma2': 4,'egami-version-info': 5}
