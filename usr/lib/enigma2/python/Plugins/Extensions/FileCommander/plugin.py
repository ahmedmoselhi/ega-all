...
class FileCommanderScreen(Screen, key_actions):
    ...
    def __init__(self, session, path_left=None):
        ...
        if config.plugins.filecommander.savedir_right.value and config.plugins.filecommander.path_right.value and os_path_isdir(config.plugins.filecommander.path_right.value):
            ...
            if config.plugins.filecommander.path_default.value and os_path_isdir(config.plugins.filecommander.path_default.value):
        ...
        self['VKeyIcon'].hide()
        self['actions'] = ActionMap(['ChannelSelectBaseActions', 'WizardActions', 'DirectionActions', 'MenuActions', 'NumberActions', 'ColorActions', 'TimerEditActions', 'InfobarActions', 'InfobarTeletextActions', 'InfobarSubtitleSelectionActions'], {'ok': self.ok,
           'back': self.exit,
           'menu': self.goMenu,
    ...
    def convertSubtitles(self, answer):
        ...
        if type(answer).__name__ != 'NoneType':
            if len(answer) > 3:
                ...
                subFile = sourceDir + testFileName
                fps = 23.976
                self.session.openWithCallback(self.subCallback, Console, _('Convert subtitle:'), ['%s convert %s "%s" "%s"' % (dmnapi_py, fps, subFile, answer)])
    ...
    def downloadNapi24Subtitle(self, answer):
        ...
        if type(answer).__name__ != 'NoneType':
            if len(str(answer)) > 3:
                ...
                subFile = sourceDir + testFileName
                fps = 23.976
                self.session.openWithCallback(self.subCallback, Console, _('Download subtitle from Napisy24 for :'), ['%s n24 %s "%s" "%s"' % (dmnapi_py, fps, subFile, answer)])
...
class FileCommanderScreenFileSelect(Screen, key_actions):
    ...
    def __init__(self, session, leftactive, selectedid):
        ...
        self['actions'] = ActionMap(['ChannelSelectBaseActions', 'WizardActions', 'DirectionActions', 'MenuActions', 'NumberActions', 'ColorActions', 'TimerEditActions', 'InfobarActions'], {'ok': self.ok,
           'back': self.exit,
           'nextMarker': self.listRight,
           'prevMarker': self.listLeft,
           'nextBouquet': self.listRight,
...
def start_from_filescan(**kwargs):
    ...
    return Scanner(mimetypes=None, paths_to_scan=[
     ScanPath(path='', with_subdirs=False)], name=pname, description=_('Open with File Commander'), openfnc=filescan_open)
